...
export const DynamicFormField = memo(function DynamicFormField({ field, value, onChange, allValues = {} }: FormFieldProps) {
    ...
    const loadDynamicOptions = async (optionsStr: string) => {
        ...
        try {
            ...
            if (optionsConfig.startsWith('ai-models:')) {
                const typeFilter = optionsConfig.split(':')[1] // chat, image, or provider name
                const data = await axiosClient.get('/ai-providers/models')

                let allModels: any[] = []

                // If it's a specific type (chat/image), filter across all providers
                if (typeFilter === 'chat' || typeFilter === 'image') {
                    ...
                    // Original logic for specific provider
    ...
    const renderField = () => {
        switch (field.type) {
            ...
            case 'select':
            case 'channel-select':
                const options = (field.type === 'channel-select' || (typeof field.options === 'string' && (field.options as string).startsWith('dynamic:')))
                    ? dynamicOptions
                    : (field.options as any[]) || []

                const selectValue = currentValue ? String(currentValue) : undefined

                const isChannelSelector = field.type === 'channel-select' || (typeof field.options === 'string' && field.options === 'dynamic:channels')
                const placeholder = loadingOptions ? "Loading channels..." : (isChannelSelector ? "Select a channel..." : "Select an option...")
