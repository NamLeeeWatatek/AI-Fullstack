-- ============================================
-- WataOmi Complete Database Schema
-- Version: 1.0.0
-- Date: 2025-11-25
-- ============================================
-- This is the single source of truth for database schema
-- Run: psql -U wataomi -d wataomi -f migrations/init_schema.sql
-- ============================================
...
CREATE TABLE workflow_executions (
    id SERIAL PRIMARY KEY,
    flow_version_id INTEGER NOT NULL REFERENCES flow_versions(id) ON DELETE CASCADE,
...
CREATE INDEX idx_workflow_executions_flow_version_id ON workflow_executions(flow_version_id);
...
-- Insert default flow templates (using simple icons to avoid encoding issues)
INSERT INTO flow_templates (name, description, category, icon, template_data, is_public, created_by, usage_count) VALUES
    ('Customer Support Bot', 'Automated customer support with AI responses and human handover', 'customer-service', 'headphones', '{"nodes": [{"id": "1", "type": "start", "position": {"x": 100, "y": 100}}, {"id": "2", "type": "aiReply", "position": {"x": 300, "y": 100}, "data": {"label": "AI Response"}}], "edges": [{"id": "e1-2", "source": "1", "target": "2"}]}', TRUE, 'system', 0),
    ('Lead Qualification', 'Qualify leads and route to sales team', 'sales', 'target', '{"nodes": [{"id": "1", "type": "start", "position": {"x": 100, "y": 100}}, {"id": "2", "type": "condition", "position": {"x": 300, "y": 100}, "data": {"label": "Check Interest"}}], "edges": [{"id": "e1-2", "source": "1", "target": "2"}]}', TRUE, 'system', 0),
    ('Order Status Checker', 'Check order status and provide updates', 'automation', 'package', '{"nodes": [{"id": "1", "type": "start", "position": {"x": 100, "y": 100}}, {"id": "2", "type": "n8nTrigger", "position": {"x": 300, "y": 100}, "data": {"label": "Check Order"}}], "edges": [{"id": "e1-2", "source": "1", "target": "2"}]}', TRUE, 'system', 0),
    ('FAQ Bot', 'Answer frequently asked questions', 'customer-service', 'help-circle', '{"nodes": [{"id": "1", "type": "start", "position": {"x": 100, "y": 100}}, {"id": "2", "type": "aiReply", "position": {"x": 300, "y": 100}, "data": {"label": "Answer FAQ"}}], "edges": [{"id": "e1-2", "source": "1", "target": "2"}]}', TRUE, 'system', 0),
    ('Appointment Booking', 'Book appointments with calendar integration', 'automation', 'calendar', '{"nodes": [{"id": "1", "type": "start", "position": {"x": 100, "y": 100}}, {"id": "2", "type": "message", "position": {"x": 300, "y": 100}, "data": {"label": "Collect Info"}}], "edges": [{"id": "e1-2", "source": "1", "target": "2"}]}', TRUE, 'system', 0)
ON CONFLICT DO NOTHING;
